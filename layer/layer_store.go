...
func (ls *layerStore) deleteLayer(layer *roLayer, metadata *Metadata) error {
	// Rename layer digest folder first so we detect orphan layer(s) if actual their removal fail
...
func (ls *layerStore) Cleanup() error {
	...
	for _, orphan := range orphanLayers {
		logrus.Debugf("removing orphan layer, chain ID: %v , cache ID: %v", orphan.Layer.ChainID(), orphan.Layer.CacheID())
		err = ls.deleteLayer(&orphan.Layer, &orphan.Metadata)
		if err != nil {
			logrus.Errorf("cannot release orphan layer: %v", err)
		}
	}
