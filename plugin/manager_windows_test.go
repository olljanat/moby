...
func newTestPlugin(t *testing.T, name, cap, root string) *v2.Plugin {
	...
	i := types.PluginConfigInterface{Socket: "plugin", Types: []types.PluginInterfaceType{iType}}
...
func (e *executorWithRunning) Create(id string, spec specs.Spec, stdout, stderr io.WriteCloser) error {
	...
	npipeAddr := filepath.Join("npipe:////./pipe/docker_plugin_", id)
	ch := make(chan struct{})
	if e.exitChans == nil {
		e.exitChans = make(map[string]chan struct{})
	}
	e.exitChans[id] = ch
	listenTestPlugin(npipeAddr, ch)
	return nil
}
...
func listenTestPlugin(npipeAddr string, exit chan struct{}) (net.Listener, error) {
	testRequires(c, testEnv.IsLocalDaemon, DaemonIsWindowsAtLeastBuild(16299)) // Named pipe support was added in RS3

	if err := os.MkdirAll(filepath.Dir(npipeAddr), 0755); err != nil {
		return nil, err
	}

	l, err := winio.ListenPipe(npipeAddr, nil)
	if err != nil {
		return nil, err
	}
	go func() {
